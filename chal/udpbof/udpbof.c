// gcc -g -O0 -fno-stack-protector udpbof.c
...
#define MAX_STRING_LENGTH 232

struct app_data
{
    uint8_t action;
    uint8_t slen;
    char sdata[MAX_STRING_LENGTH];
};

void process_packet(uint8_t *buffer, uint8_t udp_len, char *str_buf, size_t max_str_len)
{
    ...
    {
        strcpy(str_buf, "Flag functionality is permanantly disabled.\n");
        return;
    }
    if (udp_len - 2 - 8 > MAX_STRING_LENGTH) {
        strcpy(str_buf, "Nice try, no overflows this time!\n");
        return;
    }
    ...
    {
    case ACTION_PING:
        strncpy(str_buf, sdata, max_str_len);
...
int main()
{
    ...
    while (1)
    {
        ...
        process_packet(buffer + data_offset, data_len, return_str, sizeof(return_str));
